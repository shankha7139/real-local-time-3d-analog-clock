...
th=cr/8
lab=text(text='India-Time',align='center',color=color.red,height=th,pos=vector(0,cr*1.1,-ct/2),depth=ct)
aa=np.pi/2
deltaaa=-2*np.pi/12
aa+=deltaaa
for i in range(1,13):
    e=text(align='center',text=str(i),pos=vector(cr*.75*np.cos(aa),cr*.75*np.sin(aa),0),height=th,depth=ct,color=color.red)
    aa+=deltaaa
while True:
    rate(5000)
